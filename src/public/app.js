(() => {
	...
	const deleteUsers = (data = {}) =>{	
		const newUserContainer = document.getElementsByClassName('users')[0]

		while (newUserContainer.firstChild) {
			newUserContainer.removeChild(newUserContainer.lastChild);
		}
	}

	const createMenu = (data = {}) => {
		const showUsers = data.loggedUsers

		showUsers.map( username => {
			//log('entrei add')
			const usernameDiv = document.createElement('span')
			usernameDiv.setAttribute('class', 'username')
			usernameDiv.setAttribute('style','color: #fff')
			var userText = document.createTextNode(username)
			usernameDiv.appendChild(userText);
			
			const showUsersDiv = document.createElement('li')
			
			showUsersDiv.setAttribute('data-username', username)
			showUsersDiv.setAttribute('class', 'nameUser')

			showUsersDiv.appendChild(usernameDiv)

			addListUsersElement(showUsersDiv)
		})		
	}

	const addListUsersElement = (elem) => {
		usersContainer.appendChild(elem)
